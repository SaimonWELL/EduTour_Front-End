...
export function Card({ event_id, event_name, eventTags, img, description, start_date, end_date }: EventProps){
    const date_start = new Date(start_date).toLocaleDateString();
    const date_end = new Date(end_date).toLocaleDateString();
    const tagList = eventTags?.map(category => {if (category) return <div className='w-fit h-8 p-2 border-2 border-[#F6F6F6] rounded-full flex flex-row items-center justify-center gap-2'>
